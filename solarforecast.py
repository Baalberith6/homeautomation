...
def store_runtime_data(r):
    ...
    for key in r["result"]["watt_hours_period"].keys():
        if parse(key).date() == datetime.today().date():
            ...
            write_api.write(bucket=influxConfig["bucket"], record=Point("SolarForecast").field("hrly", r["result"]["watt_hours_period"][key]).time(parse(key)))

    for key in r["result"]["watt_hours_day"].keys():
        if c["debug"]:
            print(parse(key), r["result"]["watt_hours_day"][key])
        write_api.write(bucket=influxConfig["bucket"], record=Point("SolarForecast").field("dly", r["result"]["watt_hours_day"][key]).time(parse(key)))
        break
